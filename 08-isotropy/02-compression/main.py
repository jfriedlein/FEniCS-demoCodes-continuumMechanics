...
"""
----------------------------------Static Structural Analysis of a 3D Unit Cube----------------------------------

Problem Overview:
This script performs a 3D static linear elastic analysis of a unit cube using:
- Finite Element Method (FEM) for structural equilibrium
- Direct and iterative solvers for displacement and stress fields

Geometry & Mesh:
- 3D unit cube: 1.0 m x 1.0 m x 1.0 m
- Structured mesh: 3 x 3 x 3 hexahedral (box) elements

Material Model:
- Linear isotropic elasticity
  - Young's modulus (E): 200 GPa
  - Poisson's ratio (nu): 0.3
  - Lame parameters (lambda, mu) derived for 3D/plane strain conditions

Boundary Conditions:
- Dirichlet BCs: Fixed normal displacements on the left (x=0), bottom (y=0), and back (z=0) 
  boundaries to prevent rigid body motion and provide support.
- Neumann BCs: Constant compressive traction applied to the right, top, and front boundaries.

Loading:
Static triaxial compression: A uniform inward pressure of 50 MPa is applied simultaneously 
to the right (x), top (y), and front (z) faces of the cube.
...
t_p = 50.0e6 # surface traction magnitude in Pa (N/m^2)
...
# Solution function to store the computed displacement field
u = Function(V)
...
stress = project(sigma(u)/1.e6, T, solver_type="cg", preconditioner_type="petsc_amg") # project stress in MPa for output
